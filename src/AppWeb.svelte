<script>
  ...
  onMount(() => {
    ...
    if (!isTelegramAvailable) {
      authStatus = 'Для полного функционала приложение должно быть запущено внутри Telegram';
    }
  });
  ...
  async function handleTelegramAuth() {
    if (!isTelegramAvailable) {
      authStatus = 'Авторизация возможна только внутри Telegram Web App';
      return;
    }
...
<div class="container">
  <div class="glass-panel">
    ...
    {:else}
      <div class="auth-section">
        <h3 style="color: var(--primary-blue); margin-bottom: 20px;">Демо-версия приложения</h3>
        <p style="margin-bottom: 20px;">Это демонстрационная версия приложения для просмотра вне Telegram.</p>
        <p style="margin-bottom: 20px;">В полной версии вы сможете:</p>
        <ul style="text-align: left; margin-left: 20px; margin-bottom: 20px;">
          <li>Авторизоваться через Telegram</li>
          <li>Получить персональные рекомендации</li>
          <li>Отслеживать свои тренировки в реальном времени</li>
          <li>Анализировать физиологические данные</li>
        </ul>
        <button 
          class="telegram-auth-button" 
          on:click={() => showDemo = false}
          style="margin-top: 20px;"
        >
          Назад к авторизации
        </button>
